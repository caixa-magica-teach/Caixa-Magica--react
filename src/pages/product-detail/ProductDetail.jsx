...
const PERIODOS = [
  { dias: 7, label: "7 Dias", desconto: 0.9 },
  { dias: 15, label: "15 Dias", desconto: 0.8 },
  { dias: 30, label: "1 Mês", desconto: 0.7 },
];
...
export default function ProductDetail() {
  ...
  const calcularTotal = () => {
    ...
    return (
      parseFloat(brinquedo.valor_base) *
      diasSelecionados *
      (periodo?.desconto || 1)
    );
  };
  ...
  const irAnterior = () =>
    setImgAtiva((i) => (i === 0 ? totalImagens - 1 : i - 1));
  const irProximo = () =>
    setImgAtiva((i) => (i === totalImagens - 1 ? 0 : i + 1));
            ...
            <div className="detalhe-galeria">
              <div className="carrossel-wrap">
                {totalImagens > 0 ? (
                  <img
                    key={imgAtiva}
                    src={imagens[imgAtiva].imagem_url}
                    alt={brinquedo.nome}
                    className="carrossel-img"
                    onError={(e) => {
                      e.target.src =
                        "https://via.placeholder.com/500x400?text=Sem+foto";
                    }}
                  />
                ) : (
                ...
                {totalImagens > 1 && (
                  <>
                    <button
                      className="carrossel-arrow prev"
                      onClick={irAnterior}
                    >
                      ❮
                    </button>
                    <button
                      className="carrossel-arrow next"
                      onClick={irProximo}
                    >
                      ❯
                    </button>
                    <span className="carrossel-contador">
                      {imgAtiva + 1} / {totalImagens}
                    </span>
                  </>
                )}
              </div>
            ...
            {(brinquedo.descricao_curta || brinquedo.descricao_completa) && (
              <div className="detalhe-descricao-box">
                ...
                {brinquedo.descricao_curta && (
                  <p className="detalhe-descricao-curta">
                    {brinquedo.descricao_curta}
                  </p>
                )}
              ...
              <div className="periodos-opcoes">
                {PERIODOS.map(({ dias, label, desconto }) => (
                  ...
                  >
                    {label}
                    {desconto < 1 && (
                      <span className="desconto-tag">
                        -{Math.round((1 - desconto) * 100)}%
                      </span>
                    )}
                  </button>
                ))}
            ...
            {brinquedo.valor_base && (
              <div className="detalhe-preco-box">
                <span className="preco-label">
                  Total estimado ({diasSelecionados}{" "}
